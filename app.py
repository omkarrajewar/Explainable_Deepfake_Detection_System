...
def _load_model_once():
    # testing2 may provide load_model, else use tf.keras directly
    ...
    if hasattr(t2, "load_model"):
        return t2.load_model(model_path, compile=False)
    else:
...
def detect_image():
    ...
    if request.method == "POST":
        ...
        # Run your single-image explanation
        # NOTE: explain_image returns (label, p_real, explanation)
        label, p_real, explanation = t2.explain_image(str(img_path), MODEL)
